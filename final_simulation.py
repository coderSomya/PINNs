...
class FluidFlowPINN(nn.Module):
    ...
    def compute_boundary_loss(self, x_batch, y_batch):
        ...
        inner_wall_points = torch.cat([x_batch[inner_wall_mask], y_batch[inner_wall_mask]], dim=1)
        
        # No-slip condition at inner wall
        p_wall_i, v_wall_i, t_wall_i, species_wall_i = self(
            inner_wall_points[:, 0:1], 
            inner_wall_points[:, 1:2]
        )
        inner_wall_loss = torch.mean(v_wall_i**2)  # Velocity should be zero at wall
        
        # 2. Outer wall boundary conditions (r = r_o)
        outer_wall_mask = torch.abs(r - self.r_o) < 1e-4
        outer_wall_points = torch.cat([x_batch[outer_wall_mask], y_batch[outer_wall_mask]], dim=1)
        
        # No-slip condition at outer wall
        p_wall_o, v_wall_o, t_wall_o, species_wall_o = self(
            outer_wall_points[:, 0:1], 
            outer_wall_points[:, 1:2]
        )
        outer_wall_loss = torch.mean(v_wall_o**2)
        
        # 3. Inner inlet conditions (x = 0, r < r_i)
        inner_inlet_mask = (x_batch < 1e-4) & (r < self.r_i)
        inner_inlet_points = torch.cat([x_batch[inner_inlet_mask], y_batch[inner_inlet_mask]], dim=1)
        
        p_in_i, v_in_i, t_in_i, species_in_i = self(
            inner_inlet_points[:, 0:1],
            inner_inlet_points[:, 1:2]
        )
        
        inner_inlet_loss = (
            torch.mean((v_in_i - self.v_i)**2) +  # Velocity boundary condition
            torch.mean((species_in_i - self.species_i)**2)  # Species concentration BC
        )
        
        # 4. Outer inlet conditions (x = L, r_i < r < r_o)
        outer_inlet_mask = (torch.abs(x_batch - self.L) < 1e-4) & (r > self.r_i) & (r < self.r_o)
        outer_inlet_points = torch.cat([x_batch[outer_inlet_mask], y_batch[outer_inlet_mask]], dim=1)
        
        p_in_o, v_in_o, t_in_o, species_in_o = self(
            outer_inlet_points[:, 0:1],
            outer_inlet_points[:, 1:2]
        )
        
        outer_inlet_loss = (
            torch.mean((v_in_o - self.v_o)**2) +  # Velocity boundary condition
            torch.mean((species_in_o - self.species_o)**2)  # Species concentration BC
        )
        ...
        outlet_loss = (
            torch.mean(dp_dx[inner_outlet_mask]**2) +
            torch.mean(dp_dx[outer_outlet_mask]**2) +
            torch.mean(dv_dx[inner_outlet_mask]**2) +
            torch.mean(dv_dx[outer_outlet_mask]**2)
        )
...
def navier_stokes_loss(model, x, y):
    ...
    y_var = torch.tensor(y, requires_grad=True)
...
channel_length = 0.1  # meters

# Define inlet conditions
inner_inlet_velocity = 18.13  # m/s (from your sample data)
outer_inlet_velocity = -10.0  # m/s (negative for counter-flow)
...
# Initialize model with boundary conditions
model = FluidFlowPINN(
